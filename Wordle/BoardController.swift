...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
      if let numLetters = settings[kNumLettersKey] {
          self.numItemsPerRow = numLetters as! Int
      }
  }
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
      if let numGuesses = settings[kNumGuessesKey] {
          self.numRows = numGuesses as! Int
      }
  }
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
      if let wordTheme = WordTheme(rawValue: settings[kWordThemeKey] as! String) {
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
      if let isAlienWordle = settings[kIsAlienWordleKey]{
          self.isAlienWordle = isAlienWordle as! Bool
      }
  }
}
